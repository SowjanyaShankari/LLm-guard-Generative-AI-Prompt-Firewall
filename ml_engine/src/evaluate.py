...
def pick_threshold(y, scores, target_recall=TARGET_RECALL):
    ...
    # Highest threshold that still hits the target blocks the least benign traffic
...
def report(name, y, scores, threshold):
    ...
    print(f"  false alarms         {false_alarm:>7.2%}   <- benign prompts blocked")
    ...
    print(f"  [{'PASS' if recall >= TARGET_RECALL else 'FAIL'}] 95% block rate")
