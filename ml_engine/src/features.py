...
ATTACK_PATTERNS = {
    ...
    "new_instructions": re.compile(
        r"\b(new|updated|real|actual)\s+(instruction|rule|task)s?\b",
        re.I
    ),
    "asks_for_system_prompt": re.compile(
        r"\b(system\s+prompt|initial\s+instruction|your\s+(instruction|prompt|rule)s?)\b",
        re.I
    ),
    "repeat_above": re.compile(
        r"\b(repeat|print|reveal|show)\b.{0,30}\b(above|verbatim|word\s+for\s+word)\b",
        re.I
    ),
    "change_persona": re.compile(
        r"\b(you\s+are\s+now|from\s+now\s+on|act\s+as|pretend\s+to\s+be|roleplay)\b",
        re.I
    ),
    "jailbreak_name": re.compile(
        r"\b(DAN|AIM|STAN|developer\s+mode|do\s+anything\s+now)\b",
        re.I
    ),
    ...
    "unlocked": re.compile(
        r"\b(unfiltered|uncensored|unrestricted|unlocked)\b",
        re.I
    ),
    ...
    "fake_chat_marker": re.compile(
        r"(?:<\|im_(?:start|end)\|>|###\s*(?:system|assistant)|\[(?:/?INST)\])",
        re.I
    ),
    "encoding_trick": re.compile(
        r"\b(base64|rot13|hex|binary|morse)\b.{0,25}\b(decode|encode)\b",
        re.I
    ),
    "hypothetical": re.compile(
        r"\b(hypothetical|fictional|in\s+a\s+(story|game)|for\s+educational\s+purposes)\b",
        re.I
    ),
}
